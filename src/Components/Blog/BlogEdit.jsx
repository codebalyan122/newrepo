...
const getBlogById = async (id) => {
  const data = await axios.get(`http://localhost:8080/get-Blog-ById/${id}`);
  return data;
};
...
const BlogEdit = () => {
  ...
  } = useQuery("getBlogsById", () => getBlogById(id));

  const singleBlogData = BlogIdByData?.data?.data;
  //   console.log(BlogIdByData?.data?.data);
  ...
  useEffect(() => {
    setBlogContent(singleBlogData ? singleBlogData.extractedText : "");
  }, [singleBlogData]);

  const initialValues = {
    title: singleBlogData ? singleBlogData?.title : "",
    url: singleBlogData ? singleBlogData?.url : "",
    postBy: singleBlogData ? singleBlogData?.postBy : "",
    seoTitle: singleBlogData ? singleBlogData?.seoTitle : "",
    seoKeyword: singleBlogData ? singleBlogData?.seoKeyword : "",
    SeoDescription: singleBlogData ? singleBlogData?.SeoDescription : "",
  };
  if (isLoading) {
              ...
              const uploadData = async () => {
                ...
                if (file) {
                  ...
                  const Resdata = await axios.put(
                    `http://localhost:8080/edit-blog/${id}`,
                    data
                  );
                  ...
                  fileUrl = singleBlogData?.fileUrl;
                  const data = {
                    ...values,
                    fileUrl,
                    extractedText,
